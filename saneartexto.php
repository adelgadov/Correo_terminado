...
//Descargamos el csv con los parámetros necesarios para subirlo al servidor de correo.
    ...
    public function descarga_correos($archivo, $correo, $saneartexto, $i = 0) {
        header('Content-type: application/txt');
        ...
        do {
            ...
            echo preg_replace('[\s+]','',(strtolower($saneartexto -> quitarAcentos($letranombre).$saneartexto -> quitarAcentos($archivo[$i]["apellido1"]).$saneartexto -> quitarAcentos($archivo[$i]["apellido2"]).$correo.";".$archivo[$i]["password"])));
            echo "\r\n";
            $i++;
        }while ($i <count($archivo));
...
//Creamos la primera tabla para ver cómo saldrá nuestra tabla del sevidor de correo.
        ...
        do {
            ...
            echo '<tr><td>'.preg_replace('[\s+]','',(strtolower($saneartexto -> quitarAcentos($letranombre).$saneartexto -> quitarAcentos($archivo[$b]["apellido1"]).$saneartexto -> quitarAcentos($archivo[$b]["apellido2"]).$correo)))."</td><td>".$archivo[$b]["password"]."</td></tr>";
            $b++;
